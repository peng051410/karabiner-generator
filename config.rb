...
def wkey()
  JSON.pretty_generate(
    ...
    'manipulators' => [
      ...
      shell('w', 'semicolon', km('open: Sublime Text')),
      shell('w', 'l', km('open: VS Code')),
...
def okey()
  JSON.pretty_generate(
    ...
    'manipulators' => [
      ...
      shell('o', 'grave_accent_and_tilde',alfred('search processes','com.vitorgalvao.alfred.processcontrol')),
      shell('o', 'e',km('Make 2Do task')),
      shell('o', 'r',km('Make 2Do task with Safari title as name')),
      shell('o', 'q',alfred('search downloads','recentdownloads.ddjfreedom')),
      shell('o', 't',km('2Do with current url as note (read)')),
      shell('o', 'h',km('2Do with current url as note (check)')),
      shell('o', 'tab',km('2Do with current url as note (watch)')),
      shell('o', 'a',alfred('search files','nikivi.manage.wiki')),
      shell('o', 'y',alfred('search trash','com.vitorgalvao.alfred.directories')),
      ...
      shell('o', 'n',alfred('search lists','nikivi.learn.anything')),
      shell('o', 'm',alfred('search lists','nikivi.awesome.lists')),
      ...
      shell('o', 'j',alfred('search km macros','iansinnott.keyboardmaestro')),
      shell('o', 'caps_lock',alfred('search folders','nikivi.search.folders')),
      shell('o', 'n',alfred('search workflows','org.jeef.workflowdirectory')),
      shell('o', 'k',alfred('search menu bar','com.tedwise.menubarsearch')),
    ].flatten,
  )
end
...
def gkey()
  JSON.pretty_generate(
    ...
    'manipulators' => [
      ...
      key('g', 'n', 'g', ['option', 'command', 'control']),
      ...
      key('g', 'q', 'equal_sign', ['option', 'command']),
      key('g', 'm', '2', ['option', 'command', 'control']),
      key('g', 'j', 'slash', ['option', 'command']),
      shell('g', 's', km('Test')),
...
def nkey()
  JSON.pretty_generate(
    ...
    'manipulators' => [
      ...
      shell('n', 'r', alfred('search reddit', 'nikivi.search.the.web')),
